...
// Designed for 10,000+ concurrent users with connection pooling and unified caching
...
class DatabasePostgreSQL {
    constructor(monitoring = null, redisClient = null) {
        ...
        // PostgreSQL connection pool configuration - Optimized for 10,000+ concurrent users
        this.pool = new Pool({
            ...
            max: parseInt(process.env.DB_POOL_MAX) || 100, // Increased for 10k+ users
            min: parseInt(process.env.DB_POOL_MIN) || 10,  // Higher minimum for better performance
            idleTimeoutMillis: parseInt(process.env.DB_IDLE_TIMEOUT) || 20000, // Reduced idle timeout
            connectionTimeoutMillis: parseInt(process.env.DB_CONNECTION_TIMEOUT) || 2000, // Faster connection timeout
            acquireTimeoutMillis: parseInt(process.env.DB_ACQUIRE_TIMEOUT) || 1500, // Faster acquire timeout
            query_timeout: parseInt(process.env.DB_QUERY_TIMEOUT) || 10000, // Increased query timeout
            statement_timeout: parseInt(process.env.DB_STATEMENT_TIMEOUT) || 10000,
            ssl: process.env.DB_SSL === 'true' || false,
            // Additional optimizations for high load
            application_name: 'area51_bot',
            keepAlive: true,
            keepAliveInitialDelayMillis: 10000
        });

        // Initialize unified cache manager
        this.cache = redisClient ? new UnifiedCacheManager(redisClient, monitoring) : null;
        this.cacheEnabled = this.cache !== null;
    }
    ...
    // Legacy cache methods - removed to avoid conflicts with UnifiedCacheManager
    // All caching now handled by this.cache (UnifiedCacheManager)
    ...
    async createUser(telegramId, walletAddress, encryptedPrivateKey, encryptedMnemonic, username = null) {
        ...
        // Cache the user using unified cache manager
        if (result && this.cache) {
            await this.cache.set('user', telegramId, result);
        }
    }

    async getUser(telegramId) {
        // Use unified cache manager
        let user = await this.cache?.get('user', telegramId);
        
        if (!user) {
            // Optimized query selecting only needed fields
            const query = `
                SELECT telegram_id, username, wallet_address, 
                       encrypted_private_key, encrypted_mnemonic, 
...
async getUserSettings(telegramId) {
    let settings = await this.cache?.get('settings', telegramId);
    
    if (!settings) {
        // Optimized query with specific field selection
        const query = `
            SELECT telegram_id, slippage_tolerance, gas_price, 
                   custom_buy_amounts, custom_sell_amounts, turbo_mode,
    ...
    async updateUserSettings(telegramId, settingsUpdate) {
        ...
        // Update unified cache
        if (result && this.cache) {
            await this.cache.set('settings', telegramId, result);
        }
    ...
    async getUserTransactions(telegramId, limit = 50, offset = 0) {
        let transactions = await this.cache?.get('transactions', `${telegramId}:${limit}:${offset}`);
        
        if (!transactions) {
            // Optimized query using composite index idx_transactions_user_date
            const query = `
                SELECT tx_hash, token_address, token_symbol, type, amount, 
                       price, status, created_at, confirmed_at, block_number
                FROM transactions 
                WHERE telegram_id = $1 
            ...
            if (this.cache) {
                await this.cache.set('transactions', `${telegramId}:${limit}:${offset}`, transactions, 60);
            }
        }
    ...
    async getPortfolioEntries(telegramId) {
        let portfolio = await this.cache?.get('portfolio', telegramId);
        
        if (!portfolio) {
            // Optimized query using composite index and filtering active positions
            const query = `
                SELECT token_address, token_symbol, amount, buy_price, 
                       current_price, updated_at, created_at
                FROM portfolio_entries 
                WHERE telegram_id = $1 AND amount > 0
                ORDER BY updated_at DESC`;
            
            portfolio = await this.getMany(query, [telegramId]);
            
            if (this.cache) {
                await this.cache.set('portfolio', telegramId, portfolio, 120);
            }
        }
        ...
        setTimeout(async () => {
            try {
                ...
                this.pool = new Pool({
                    ...
                    acquireTimeoutMillis: parseInt(process.env.DB_ACQUIRE_TIMEOUT) || 1500,
                    keepAlive: true,
                    keepAliveInitialDelayMillis: 10000
                });
    ...
    async deleteUser(telegramId) {
        ...
        // Invalidate all user-related cache using unified cache manager
        if (this.cache) {
            await this.cache.invalidateUserCache(telegramId);
            await this.cache.invalidateStaticUserData(telegramId);
        }
    ...
    async trackUserActivity(telegramId) {
        ...
        try {
            ...
            if (this.cache) {
                await this.cache.delete('user', telegramId);
            }
        } catch (error) {
