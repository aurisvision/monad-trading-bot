...
class NavigationHandlers {
    ...
    async handleStart(ctx) {
        ...
        try {
            // Check for start parameters from Inline Mode
            const startPayload = ctx.startPayload || ctx.payload;
            if (startPayload) {
                await this.handleInlineStartParameter(ctx, startPayload);
                return;
            }
    ...
    /**
     * Handle start parameters from Inline Mode
     */
    async handleInlineStartParameter(ctx, payload) {
        const userId = ctx.from.id;
        ...
        try {
            ...
            if (!user || !user.wallet_address || user.wallet_address === 'pending_wallet_creation') {
                await ctx.reply('❌ Please set up your wallet first using /start');
                return;
            }

            // Parse the payload: format is "action_tokenAddress"
            ...
            const tokenAddress = parts.slice(1).join('_'); // In case address contains underscores
            ...
            this.monitoring.logInfo('Processing inline start parameter', { 
                userId, 
                action, 
            ...
            switch (action) {
                case 'buy':
                    await this.handleInlineBuyRedirect(ctx, tokenAddress);
                    break;
                case 'sell':
                    await this.handleInlineSellRedirect(ctx, tokenAddress);
                    break;
                case 'autobuy':
                    await this.handleInlineAutoBuyRedirect(ctx, tokenAddress);
                    break;
                default:
                    await ctx.reply('❌ Unknown action type');
                    break;
            }

        } catch (error) {
            this.monitoring.logError('Inline start parameter handling failed', error, { userId, payload });
            await ctx.reply('❌ Error processing request. Please try again.');
        }
    }

    /**
     * Handle inline buy redirect
     */
    async handleInlineBuyRedirect(ctx, tokenAddress) {
        try {
            // Get token info first
            const tokenInfo = await this.monorailAPI.getTokenInfo(tokenAddress);
            if (!tokenInfo || !tokenInfo.token) {
                await ctx.reply('❌ Token not found or invalid');
                return;
            }

            // Show buy interface using existing TradingInterface
            if (this.mainBot && this.mainBot.tradingInterface) {
                await this.mainBot.tradingInterface.handleBuyInterface(ctx, tokenAddress);
            } else {
                await ctx.reply('❌ Trading interface not available');
            }

        } catch (error) {
            this.monitoring.logError('Inline buy redirect failed', error, { tokenAddress });
            await ctx.reply('❌ Error loading buy interface');
        }
    }

    /**
     * Handle inline sell redirect
     */
    async handleInlineSellRedirect(ctx, tokenAddress) {
        try {
            // Get token info first
            const tokenInfo = await this.monorailAPI.getTokenInfo(tokenAddress);
            if (!tokenInfo || !tokenInfo.token) {
            ...
            // Show sell interface using existing TradingInterface
            if (this.mainBot && this.mainBot.tradingInterface) {
                await this.mainBot.tradingInterface.handleSellInterface(ctx, tokenAddress);
            } else {
                await ctx.reply('❌ Trading interface not available');
            }

        } catch (error) {
            this.monitoring.logError('Inline sell redirect failed', error, { tokenAddress });
            await ctx.reply('❌ Error loading sell interface');
        }
    }

    /**
     * Handle inline auto-buy redirect
     */
    async handleInlineAutoBuyRedirect(ctx, tokenAddress) {
        try {
            // Execute auto-buy directly using existing logic
            await this.processTokenAddress(ctx, tokenAddress);

        } catch (error) {
            this.monitoring.logError('Inline auto-buy redirect failed', error, { tokenAddress });
            await ctx.reply('❌ Error executing auto-buy');
        }
    }
...
**Example:** \`0x1234567890123456789012345678901234567890\``;
    ...
    async handleTextMessage(ctx) {
        ...
        await ctx.reply('🔍 *Search for tokens by name, symbol, or address*\n\nJust type the token name (e.g., "USDC", "Bitcoin") or paste a contract address to search.\n\nOr use the menu buttons below to navigate.', {
            parse_mode: 'Markdown'
        });
    }
...
Confirm this transaction?`, {
    ...
    /**
     * Handle group commands (@MonAreaBot search/buy)
     */
    async handleGroupCommands(ctx, messageText) {
        const userId = ctx.from.id;
        ...
        try {
            ...
            if (!match) {
                return; // Not a bot mention, ignore
            }
            
            const command = match[1].trim();
            
            // Parse search command: @MonAreaBot search [token]
            const searchMatch = command.match(/^search\s+(.+)$/i);
            if (searchMatch) {
                const tokenQuery = searchMatch[1].trim();
                await this.handleGroupTokenSearch(ctx, tokenQuery, username);
                return;
            }
            
            // Parse buy command: @MonAreaBot buy [token] [amount]
            const buyMatch = command.match(/^buy\s+(\S+)\s+(\d+(?:\.\d+)?)$/i);
            if (buyMatch) {
                const tokenQuery = buyMatch[1].trim();
                const amount = parseFloat(buyMatch[2]);
                await this.handleGroupTokenBuy(ctx, tokenQuery, amount, username);
                return;
            }
    ...
    /**
     * Handle group token search command
     */
    async handleGroupTokenSearch(ctx, tokenQuery, username) {
        const userId = ctx.from.id;
        ...
        try {
            ...
            const token = searchResults.tokens[0];
            ...
            // Format token information for group display
            const tokenData = tokenInfo.token;
            const tokenPriceUSD = parseFloat(tokenData.usd_per_token || '0');
            const tokenPriceInMON = parseFloat(tokenData.mon_per_token || '0');
            const confidence = parseInt(tokenData.pconf || '100');
...
💡 **للشراء استخدم:** \`@MonAreaBot buy ${tokenData.symbol || tokenQuery} [amount]\``;
    ...
    /**
     * Handle group token buy command
     */
    async handleGroupTokenBuy(ctx, tokenQuery, amount, username) {
        const userId = ctx.from.id;
        ...
        try {
            ...
            // Execute trade using unified trading engine
            const TradingInterface = require('../trading/TradingInterface');
            const tradingDependencies = {
                database: this.database,
                monorailAPI: this.monorailAPI,
            ...
            // Determine trade type based on user settings
