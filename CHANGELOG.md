...
## [v2.3.0] - 2025-09-13 - Gas Price & Database Critical Fixes

### 🚨 CRITICAL GAS PRICE FIXES
**PROBLEM**: Users experiencing transaction failures due to low gas prices (20 Gwei minimum)
**SOLUTION**: Updated minimum gas price to 50 Gwei for guaranteed transaction success

#### Database Updates
- **EXISTING USERS**: Updated 3 users from 20 Gwei to 50 Gwei via SQL commands
- **NEW USERS**: Fixed `createUserSettings()` to set proper defaults:
  - `gas_price = 50000000000` (50 Gwei) for Buy transactions
  - `auto_buy_gas = 50000000000` (50 Gwei) for Auto Buy transactions  
  - `sell_gas_price = 50000000000` (50 Gwei) for Sell transactions
- **CACHE CLEARING**: Executed `redis-cli FLUSHALL` to force settings refresh

#### Issues Resolved
1. **Gas Too Low Error**: 20 Gwei causing "transaction underpriced" failures
2. **New User Defaults**: createUserSettings() only setting turbo_mode, missing gas values
3. **Cache Persistence**: Old settings cached in Redis preventing display updates
4. **Wallet Recreation**: Users deleting/recreating wallets still getting 20 Gwei defaults

#### Technical Changes
```sql
-- Fixed existing users
UPDATE user_settings SET gas_price = 50000000000 WHERE gas_price = 20000000000;

-- Enhanced createUserSettings function
INSERT INTO user_settings (
    telegram_id, gas_price, sell_gas_price, auto_buy_gas, 
    slippage_tolerance, sell_slippage_tolerance, auto_buy_slippage,
    auto_buy_enabled, auto_buy_amount, turbo_mode
) VALUES ($1, 50000000000, 50000000000, 50000000000, 5.0, 5.0, 5.0, false, 0.1, false)
```

#### Validation Results
- ✅ **Buy Settings**: Now shows 50 Gwei for all users
- ✅ **Auto Buy Settings**: Now shows 50 Gwei for all users  
- ✅ **Sell Settings**: Already correct at 50 Gwei
- ✅ **New Users**: Get 50 Gwei defaults immediately upon wallet creation

### 🔧 Portfolio Service Recovery
- **CORRUPTION FIX**: Restored corrupted `portfolioService.js` from git history
- **ENCODING ISSUES**: Fixed file corruption that caused module loading failures
- **FORMATTING**: Updated portfolio display with proper Markdown formatting
